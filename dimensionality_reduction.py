...
class DimensionalityReduction:
    ...
    def umap(X, components=3, **kwargs):
        ...
        '''
        if not UMAP_PRESENT:
            return DimensionalityReduction.tsne(X, components=components, **kwargs)
    ...
    def execute(X, **kwargs):
        rtype = kwargs.get("rtype", DimensionalityReduction.UMAP)
        ...
        if rtype == DimensionalityReduction.PCA:
            return DimensionalityReduction.pca(X, components)

        elif rtype == DimensionalityReduction.TSNE:
            return DimensionalityReduction.tsne(X, components, **kwargs)

        return DimensionalityReduction.umap(X, components, **kwargs)
